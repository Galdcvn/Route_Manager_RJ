...
export function AuthModal({ open, onClose }: AuthModalProps) {
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    if (result.error) {
      ...
      toast({ type: 'success', message: t('auth.resetEmailSent') })
      handleClose()
    }
  }
        ...
        <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl sm:p-8">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-xl font-bold text-navy">
              {tab === 'reset' ? t('auth.resetPassword') : tab === 'login' ? t('auth.loginTitle') : t('auth.signupTitle')}
            </h2>
            <button
              onClick={handleClose}
          ...
          <form onSubmit={tab === 'reset' ? handleResetPassword : handleSubmit} className="flex flex-col gap-4">
            {tab === 'signup' && (
              <div>
            ...
            {tab === 'reset' && (
              <p className="text-xs text-slate-400">{t('auth.resetInstructions')}</p>
            )}
            ...
            {tab === 'login' && (
              <button
                ...
                className="self-end text-xs font-medium text-sky transition hover:text-sky/80"
              >
          ...
          {tab === 'reset' && (
            <button
              type="button"
              onClick={() => setTab('login')}
              className="mt-4 w-full text-center text-xs font-medium text-slate-500 transition hover:text-navy"
            >
